...
Если `git` отсутствует, то предварительно установим его:
...
Предварительно настроим конфигурацию будущего сервера, которая хранится в `config.json`, например, с помощью редактора `nano`:
